...
给 Emby Server 用的 [Bangumi 番组计划](https://bgm.tv) 动画 / 特摄元数据刮削插件。
剧集、季度、分集、电影、封面、制作人员与演员全部走 Bangumi 官方 API。
...
- 修复 `242 4K.mp4` 被 Emby 拆成 `S02E42` 后误匹配的情况：裸三位集号需由 Bangumi 已播出的唯一 `sort` 精确确认，再保留连续集号并归入默认季度。例：吞噬星空宇宙篇 `242 → sort 242 / ep 157`，不再误用 `ep 42 / sort 127`。
- 这条纠错会跳过旧的错误分集 ID 和虚拟季度 ID；刷新后仍保持正确映射。显式 `S02E42`、`Season 2` 文件夹、手动偏移、合并集数、年份与分辨率不套用此规则。
- WatchTarget 增加 `ResolverVersion=2`，配套 bridge 在新观看事件上报前也校验已有分集 ID；解析失败时自动等待，不继续上报旧目标。升级时请同步更新 bridge。媒体文件无需重命名。
...
- 首页新增「最近观看」并与 Emby 原生媒体模块统一卡片尺寸、间距、居中标题与焦点样式。
- 保留横向滚动、已看标记与集标题，长标题自动省略，不改变「继续观看」原生模块。

## 功能

- **观看同步只读匹配接口（v1.3.0）**：`GET /Bangumi/Items/{id}/WatchTarget` 需 Emby 用户鉴权并检查条目访问权限。
  给点格子桥接器解析没有 `BangumiEpisode` 的分集，复用父级 Bangumi ID、季度与连续集号解析，
  不刷新元数据、不改观看状态、不依赖媒体库名或 ani-rss 订阅。自动编号冲突、合并多集、未播出、
  没有可靠 ID 时宁可返回未解析；不使用纯位置猜测。插件提供匹配，真正写入 Bangumi 仍由 bridge 完成。
...
- **动画与特摄双类型检索**：默认先搜索 Bangumi 动画（`type=2`）；没有高置信结果时再搜索
  三次元（`type=6`），覆盖奥特曼、假面骑士、超级战队等特摄剧集与电影。手动「识别」会同时
  列出两类候选；季度、分集和续集链会锁定根条目类型，不会串到同名动画或真人改编。
...
- **条目页面独立角色栏（Bangumi UI）**：Emby 原生「演职人员」把角色和 staff 混在同一行卡片里，
 ...
 - **增强选集面板**：剧集 / 季 / 分集页面将原生选集替换为可切换的**封面卡片**与**数字网格**；按集号跳转、
   定位下一集 / 续看进度、按季记忆浏览位置；卡片主体一键播放，右上角可快速标记已看，详情按钮仍进入 Emby 原生分集页，
   因此版本、音轨、字幕和外部播放器链路保持不变。首页另外增加「最近观看」栏，按 Emby 的最后播放时间显示已看过的分集和电影。
...
| 响应缓存时长 | 30 分钟 | 0 = 不缓存 |
| 同时支持特摄 / 三次元条目 | 开 | 动画没有高置信匹配时再搜索 Bangumi `type=6`；手动「识别」同时列出两类候选 |
| 搜索结果数量 | 20 | |
...
| 增强选集面板 | 开 | 封面 / 数字视图自由切换，按集号跳转、定位下一集 / 续看进度；卡片主体一键播放，右上角快速标记已看，详情按钮进入原生分集页。浏览位置按浏览器、Emby 服务器和用户隔离保存。关闭即保留原生选集 |
